...
import SingleProjectLeftSidebar from './SingleProjectLeftSideBar';
import SingleProjectCenterContent from './SingleProjectCenterContent';
import SingleProjectRightSidebar from './SingleProjectRightSidebar';
import ProjectImageGallery from './ProjectImageGallery';

interface SingleProjecCtontentProps {
    project: Project
}
const SinglePortfolioContent: React.FC<SingleProjecCtontentProps> = ({ project }) => {
    const localDateTime = new Date(project.date).toLocaleString(undefined, {
        year: 'numeric',
        month: 'long',
        day: 'numeric',
        hour: 'numeric',
    ...
    const mainImageUrl = project.images?.[0]?.asset ? urlFor(project.images[0]) : '/fallback.jpg'
            ...
            <section className={`outer-spacing `}>
                <div className="main-container">
                    <div className="w-fit pb-[40px]">
                        <h2 className='text-[32px] p-1 bg-pastel-yellow w-fit text-black inline leading-[calc(100%_+_18px)]'>Project: {project.title}</h2>
                    ...
                    <div className="mt-10 flex items-start justify-between flex-col xl:flex-row gap-y-4">
                        <SingleProjectLeftSidebar publishTime={localDateTime} tools={project.tools} technologies={project.technologies} />
                        <div className="order-1 xl:order-2 xl:px-4 shrink-[2] grow">
                            <SingleProjectCenterContent title={project.title} description={project.description} content={project.content} />
                            <ProjectImageGallery images={project.images} />
                        </div>
                        <SingleProjectRightSidebar publishTime={localDateTime} description={project.description} author={project.authorName} service={project.service} liveLink={project.liveLink} githubLink={project.githubLink} views={project.views} />
                    </div>
                </div>
            </section>
